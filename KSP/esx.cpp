...
int
compute_priority(RoadNetwork *rN, Edge &e, vector<int> &bounds, unordered_set<Edge, boost::hash<Edge>> &deletedEdges);

int compute_priority_maxpt(RoadNetwork *rN, Edge &e, vector<int> &bounds,
                           unordered_set<Edge, boost::hash<Edge>> &deletedEdges);

int compute_priority_minpt(RoadNetwork *rN, Edge &e, vector<int> &bounds,
                           unordered_set<Edge, boost::hash<Edge>> &deletedEdges);
...
vector<Path> esx(RoadNetwork *rN, NodeId source, NodeId target, unsigned int k, double theta)
{
    ...
    while (resPaths.size() < k && possible)
    {
        ...
        while (olRatio > theta)
        {
            ...
            if (newP.nodes.empty())
            { // If astar_limited did not find a path
            ...
            {
                if (newP.overlap_ratio(rN, resPath) > theta)
                {
                    check = true;
                    break;
                }
...
/*
 *
 *	Implementation of the ESX-C KSP
 *
 */
    ...
    else
    {
        ...
        while (resPaths.size() < k && possible)
        {
            ...
            while (olRatio > theta)
            {
                ...
                for (auto &resPath: resPaths)
                {
                    double currentTheta = newP.overlap_ratio(rN, resPath);
...
int compute_paths_through(RoadNetwork *rN, Edge &e, vector<int> &bounds,
                          unordered_set<Edge, boost::hash<Edge>> &deletedEdges)
{
    int strength2 = 0;
    EdgeList::iterator iterAdj;
    ...
    for (unsigned int source: sources)
    {
        ...
        {
            Path tempP = astar_limited(rN, source, target, bounds, deletedEdges);
