...
//    printf("\nInsertion using %f sec.\n", t);
    ...
    vector<int> res;
    double *buf = new double[2 * dim];
    data[5]->get_mbr(buf);
    for (int i = 0; i < data[0]->dim; ++i)
    {
        buf[2 * i + 1] += 0.1f;
    }
    tree->windows_query(buf, res);
...
void test(int dim, int page_len, int data_num)
{
    ...
    char n1[] = "insert.rt", n2[] = "bulkload.rt";
    test_insert(&(n1[0]), dim, page_len, data, data_num);

    test_bulkload(&(n2[0]), dim, page_len, data, data_num);
...
//{
//    setseed((unsigned long) time(nullptr));
//
//    ConfigTool cfg;
