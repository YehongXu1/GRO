...
enum INSERT_RESULT
{
    INSERT_SPLIT, INSERT_REINSERT, INSERT_NONE
};
enum INTERSECT_RESULT
{
    INTERSECT_OVERLAP, INTERSECT_INSIDE, INTERSECT_NONE
};
...
struct mbr_compare_less
{
    ...
    mbr_compare_less(T **mbrs, int *ids, int the_dim, int tag) : all_mbr(mbrs), all_id(ids), comp_dim(the_dim),
                                                                 lu_tag(tag) {}
...
struct mbr_compare_center_less
{
    ...
    mbr_compare_center_less(T **mbrs, int *ids, T *the_center, int the_dim) : all_mbr(mbrs), all_id(ids),
                                                                              center(the_center), comp_dim(the_dim) {}
...
template<template<typename TP> class Data = TreeDataP, typename T = float>
class RStarTree;
...
public:
    ...
    explicit RSTNode(RStarTree<Data, T> *rt);

    virtual ~RSTNode();

    virtual void flush() = 0; //flush this to disk
    virtual void windows_query(T *q_mbr, vector<int> &res) = 0;
    ...
    virtual Data<T> *
    get(int index, Data<T> *data_buf) = 0; //returns the i-th object in this sub-tree rooted at this node
...
public:
    ...
    void windows_query(T *q_mbr, vector<int> &res);
...
public:
    ...
    void bulkload_str(Data<T> **the_data_array, int num_data,
                      double load_factor); //bulk load all data, using Sort-Tile-Recursive method
};

template<template<typename TP> class Data, typename T>
void RStarTree<Data, T>::windows_query(T *q_mbr, vector<int> &res)
{
    //we make sure root node is loaded
    load_root();

    root->windows_query(q_mbr, res);
}
...
int RSTNode<Data, T>::split(T **mbr_array, int *mbr_id, int *split_buf)
{
    ...
    T min_over = numeric_limits<T>::max();
    T min_dead = numeric_limits<T>::max();
    int split_point = -1;
...
int RSTNonLeafNode<Data, T>::choose_subtree(T *mbr)
{
    ...
    } else if (inside_count > 1)
    {
        ...
        T min_area = numeric_limits<T>::max();
        for (vector<int>::iterator iter = inside_id.begin(); iter != inside_id.end(); ++iter)
        {
            T the_area = area<T>(this->dim, entry_mbr[*iter]);
    ...
    } else
    {
        ...
        if (child_is_leaf)
        {
            ...
            T min_overlap = numeric_limits<T>::max();
            T min_enlarge = numeric_limits<T>::max();
            T min_area = numeric_limits<T>::max();
            ...
            for (int i = 0; i < this->entry_num; i++)
            {
                ...
                T the_area = area(this->dim, entry_mbr[i]);
                T the_enlarge = area(this->dim, mbr_buf) - the_area;

                //calculate the delta overlap before and after enlargement
                T before_overlap = (T) 0;
                T after_overlap = (T) 0;
...
template<template<typename TP> class Data, typename T>
void RSTNonLeafNode<Data, T>::windows_query(T *q_mbr, vector<int> &res)
{
    ...
    for (int sub_id = 0; sub_id < this->entry_num; ++sub_id)
    {
        ...
        for (int i = 0; i < 2 * this->dim; i += 2)
        {
            if ((q_mbr[i] > entry_mbr[sub_id][i + 1]) ||
                (q_mbr[i + 1] < entry_mbr[sub_id][i]))
            {
                flag = false;
                break;
            }
        ...
        if (flag) this->children[sub_id]->windows_query(q_mbr, res);
    }
...
template<template<typename TP> class Data, typename T>
void RSTLeafNode<Data, T>::windows_query(T *q_mbr, vector<int> &res)
{
    for (int j = 0; j < this->entry_num; ++j)
    {
        bool flag = true;
        ...
        {
            if ((q_mbr[2 * i] > this->data[j]->data[i]) ||
                (q_mbr[2 * i + 1] < this->data[j]->data[i]))
            {
                flag = false;
                break;
            }
        }
        if (flag) res.push_back(this->data[j]->id);
    }
}
...
INSERT_RESULT RSTLeafNode<Data, T>::insert(Data<T> *d, RSTNode<Data, T> **sn)
{
    ...
    if (this->entry_num == (this->capacity - 1)) //back to original
    {
        if (this->the_tree->reinsrt_lvl[0] == false)
        {
            ...
            sort(sort_center, sort_center + this->entry_num,
                 mbr_compare_center_less<T>(entry_mbr, entry_id, center, this->dim)); //lower
...
//RStarTree
template<template<typename TP> class Data, typename T>
RStarTree<Data, T>::RStarTree(const char *fname, int dimension, int p_len, const char *path, PageFile::c_policy pol,
                              int c_size, bool force_new)
